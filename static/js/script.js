...
function compute_filter() {
    ...
    products.forEach(function(element) {
        ...
        // Gender and age group are optional to enable; currently disabled as no longer requested.
...
function toggle_filter(filter_button) {
    var filter_options = {
        ...
        // Gender and group have been disabled; enable by uncommenting when and if ever required later.
            ...
            for(var shoe_info of [first_shoe_info, "middle_section", second_shoe_info]) {
                if(shoe_info == "middle_section") {
                    ...
                    if(environmental_impact != null) {
                        ...
                        // If the environmental rating is left blank, display 5 grey circles.
                        if(!environmental_impact.rating) {
                            var ball_count = 5;
                            if(shoe_info == second_shoe_info)
                                ball_wrapper.style.textAlign = "right"
                            var color_base = "custom_scoring_balls_"
                            ball_inner.classList = color_base + "grey"
                            for(var i = 0; i < ball_count; i++) {
                                var ball = document.createElement("div")
                                ball_inner.appendChild(ball)
                            }
                        }
                        else if(environmental_impact.rating != null) {
